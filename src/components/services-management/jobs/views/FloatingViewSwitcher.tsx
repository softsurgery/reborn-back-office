...
export const FloatingViewSwitcher: React.FC<FloatingViewSwitcherProps> = ({
  ...
  return (
    <div
      className={cn(
        "flex items-center gap-1 p-1.5 rounded-xl bg-background/90 dark:bg-card/90 backdrop-blur-xl border border-border/80 shadow-sm hover:shadow-primary/15 transition-all duration-300 animate-in fade-in zoom-in-95",
        className,
      )}
    >
      {buttons.map((button) => (
        <button
          ...
          className={cn(
            "flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs transition-all duration-200 select-none",
